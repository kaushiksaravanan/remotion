// ─── src/SendblueStatics/Static3VSLvsIMessage.tsx ────────────────────────────
// 1080×1080 static ad — bold typography comparison: "Passive. vs. Immediate."
	...
	return (
		<svg width={width} height={height} viewBox="0 0 119 24" fill="none">
			...
			<path
				...
				fill="white"
			/>
			<path d="M92.5051 19.3736V4.6264H94.772V19.3736H92.5051Z" fill="white" />
			<path
...
export type Static3VSLvsIMessageProps = Record<string, never>;

export const Static3VSLvsIMessage: React.FC<Static3VSLvsIMessageProps> = () => {
	...
	// ── "Passive." slides up ─────────────────────────────────────────────────
	const passiveSpring = spring({frame, fps, config: SPRINGS.gentle});
	const passiveY = interpolate(passiveSpring, [0, 1], [40, 0]);
	const passiveOpacity = interpolate(
		frame,
	...
	// ── "vs." fades in ───────────────────────────────────────────────────────
	const vsDelay = Math.round(0.5 * fps);
	...
	// ── "Immediate." slides up ───────────────────────────────────────────────
	const immediateDelay = Math.round(0.8 * fps);
	const immediateSpring = spring({
		frame: frame - immediateDelay,
		fps,
		config: SPRINGS.gentle,
	});
	const immediateY = interpolate(immediateSpring, [0, 1], [40, 0]);
	const immediateOpacity = interpolate(
		frame - immediateDelay,
		[0, Math.round(0.6 * fps)],
		[0, 1],
		{extrapolateLeft: 'clamp', extrapolateRight: 'clamp'},
	);
	...
	// ── Subtitle ─────────────────────────────────────────────────────────────
	const subtitleDelay = Math.round(1.5 * fps);
	...
	// ── CTA ──────────────────────────────────────────────────────────────────
	const ctaDelay = Math.round(2 * fps);
	...
	return (
		<AbsoluteFill>
			{/* Dark background */}
			<AbsoluteFill style={{background: '#0A0C10', opacity: bgOpacity}} />
			<AbsoluteFill
				style={{
					background:
			...
			>
				...
				<div
					style={{
						...
						transform: `translateY(${passiveY}px)`,
						marginBottom: 16,
					}}
				>
					<span
				...
				{/* "vs." — small, dim */}
				<div style={{opacity: vsOpacity, marginBottom: 16}}>
					<span
						style={{
							fontFamily: interFont,
							fontSize: 28,
				...
				<div
					style={{
						opacity: immediateOpacity,
						transform: `translateY(${immediateY}px)`,
						marginBottom: 56,
					}}
				>
					<span
				...
				>
					...
					>
						Replace your VSL funnel with a live{' '}
						<span style={{color: COLORS.sendblue, fontWeight: 700}}>
							iMessage
						</span>{' '}
						conversation.
					</span>
				...
				>
					...
					>
						Replace your VSL with iMessage
					</span>
				</div>
			</AbsoluteFill>
