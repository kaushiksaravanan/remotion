// ─── src/SendblueStatics/Static5DeliveryChannel.tsx ──────────────────────────
// 1080×1080 static ad — text-driven "delivery channel" concept
...
export type Static5DeliveryChannelProps = Record<string, never>;

// ── Three copy lines — the visual IS the text ────────────────────────────────
const LINES = [
	{text: "Your leads don't check email.", highlight: false},
	{text: "They don't pick up calls.", highlight: false},
	{text: 'But they read every iMessage.', highlight: true},
] as const;

export const Static5DeliveryChannel: React.FC<
	Static5DeliveryChannelProps
> = () => {
	...
	// ── Three copy lines — staggered ─────────────────────────────────────────
	const lineBaseDelay = Math.round(0.9 * fps);
	const lineStagger = Math.round(0.4 * fps);

	// ── CTA ──────────────────────────────────────────────────────────────────
	const ctaDelay = lineBaseDelay + LINES.length * lineStagger + Math.round(0.3 * fps);
	...
	return (
		<AbsoluteFill>
			...
			>
				...
				>
					{LINES.map((line, i) => {
						const delay = lineBaseDelay + i * lineStagger;
						const s = spring({
							frame: frame - delay,
							fps,
							config: SPRINGS.gentle,
